...
import { getMessages, getGeneralMessages, sendMessage } from '../../../services/messages';
        ...
        return (
            <div className='general-chat-primary'>
                <div className='general-chat-primary-profile'>
                    <h3>asd123</h3>
                    <i style={{marginLeft: "5px", marginTop: "3px"}} class="fas fa-angle-down"></i>
                </div>
                <div style={{justifyContent: "center", width: "100%", margin: "0", borderTop: "0.01em solid #2b2b2b3f"}} className="type-set">
                    ...
                    <div style={{width: "120px"}} onClick={() => setType(2)} className={ type === 2 ? "type-card active" : "type-card"}>
                        <h3>Solicitudes (2)</h3>
                    </div>
                ...
                <div className='general-chat-card-container'>
                    ...
                    {messages && messages.map((message) => {
                        return (
                            <div onClick={() => handleClick(message)} className='general-chat-card'>
                                <div>
                                    <img style={{backgroundColor: "#f36196", width: "40px", height: "40px"}} className='avatar-only-head' src={message.avatar} />
                                </div>
                                ...
                                <div style={{width: "65%", textAlign: "right"}}>
                                    <h3 style={{fontSize: "14px", position: "relative", top: "-10px"}}>{formatHour(message.createdAt)}</h3>
                                </div>
                            </div>
                        )
                    })}
        ...
        return (
            <div className='general-chat-selected'>
                        ...
                        {selectedMessages && selectedMessages.map((message, index) => {
                            ...
                            return (
                                <div>
                                    ...
                                    <div style={{justifyContent: message.name === user.name && "right"}} className='general-chat-selected-message'>
                                        {message.name != user.name && <div>
                                            <img className='avatar-only-head' src={message.avatar} alt="" />
                                        </div>}
                ...
                <div className='general-chat-selected-input'>
                    <i style={{width: "40px", textAlign: "center"}} class="fas fa-smile"></i>
                    <input 
                        type="text" 
                        value={message}
    ...
    function renderSecondary() {
        return (
            <div className='general-chat-secondary'>
                {selectedMessages === null ? <div style={{textAlign: "center"}}>
                    <i style={{fontSize: "34px", marginBottom: "10px"}} class="fas fa-envelope-open-text"/>
                    <h3>Tus mensajes</h3>
                    <p style={{color: "darkgray", fontSize: "13px", marginTop: "5px", marginBottom: "5px"}}>Envía fotos y mensajes privados a un amigo o grupo.</p>
                    <button className="chat-button-view-chat">Enviar mensaje</button>
                </div> : renderMessageSelected()}
            </div>
        )
    }
