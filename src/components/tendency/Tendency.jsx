...
export default function Tendency() {
    ...
    function getType() {
        if(type === 0) {
            return vodTendency.map((vod, index) => <Link style={{textDecoration: "none"}} to={"/vod/" + vod._id}><TendencyCard tendency={index + 1} title={vod.stream_title} streamer={vod.streamer} views={vod.views} createdAt={vod.createdAt} categorie={vod.stream_category} tags={vod.stream_tag} image={vod.stream_thumbnail}/></Link>)
        }

        if(type === 1) {
            return clipTendency.map((vod, index) => <Link style={{textDecoration: "none"}} to={"/clip/" + vod._id}><ClipTendencyCard tendency={index + 1} title={vod.clipName} streamer={vod.name} views={vod.views} createdAt={vod.createdAt} categorie={vod.stream.stream_category} tags={vod.stream_tag} image={vod.cover}/></Link>)
        }
    }
        ...
        <div className="tendency-body">
            ...
            <div className="tendency-container">
                ...
                <div>
                    ...
                    <div style={{margin: "0", justifyContent: "left", marginTop: "15px"}}  className="type-set">
