...
import { getStreamsRecommended } from "../../services/stream"
import NavbarLeftMobile from "./navbarLeftMobile"


var activeNormal = 0;

export default function NavbarLeft({isMobile, tyExpand, tyExpanded, setExpanded, tyDashboard}) {
    ...
    useEffect(() => {
        ...
        if(window.location.pathname.split("/")[1] === user.name) {
            setDashboard(false);
        }

        if(window.location.pathname === "/" + user.name + "/dashboard/streammanager") {
            setDashboard(true);
        }
        if(window.location.pathname === "/" + user.name + "/dashboard/home") {
            setDashboard(true);
        }

        if(window.location.pathname != "/plataform/clips") {
            //Cancel window scroll event
            window.onscroll = function(e) {console.log("")};
        }
    ...
    useEffect(() => {
        const fetchData = async () => {
            const result = await getStreamsRecommended(token)
            if(result != undefined && result != null) {
    ...
    function getNormalNavbar() {
        ...
        if(dashboard === true) {
            return (
                <div>
                    <div style={{display: "flex", alignItems: "center", height: "105px"}} className="navbarleft-dashboard-title">
                        <div style={{textAlign: "center", position: "relative", top: "5px"}} className="other-div-nav">
                            <img style={tyExpanded ? {cursor: "pointer", position: "relative", width: "65px", borderRadius: "200px"} : {borderRadius: "100px", width: "40px"}} src={user.avatar} alt="" />
                        </div>
                        <div style={{marginLeft: "15px", textAlign: "left", position: "relative", top: "5px"}}>
                            <h3 style={{fontWeight: "800", fontSize: "16px"}}>{user && user.name}</h3>
                            <h1 style={{fontSize: "18px", marginTop: "5px", color: "#ededed"}}>{user.followers && user.followers.length}</h1>
                            <p style={{fontSize: "11px", marginTop: "5px", color: "lightgray"}}>Total de seguidores</p>
                        </div>

                    </div>

                    <div style={{width: "88%", margin: "10px auto", height: "1px", backgroundColor: "#ffffff1a", marginTop: "10px", marginBottom: "10px"}}/>
                   
                    <Link to={"/" + user.name + "/dashboard/home"} onClick={() => activeNormal = 0} className="navbarleft-link">
                        <li style={{backgroundColor: activeNormal === 0 && "#3b3b3b", borderLeft: activeNormal === 0 && "3px solid #f36196"}} class={active === 0 ? "navbaraccount-li has-subnav nav-active" : "navbaraccount-li has-subnav"}>
                            <i style={{fontSize: "14px", marginTop: "13px", color: "#ededed"}} class="fa fa-home fa-2x"/>
                            <span style={{color: activeNormal === 0 && "#f36196"}} class="nav-text">
                                Inicio
                            </span>
                        </li>
                    </Link>

                    <Link to={"/" + user.name + "/dashboard/streammanager"} onClick={() => activeNormal = 1}  className="navbarleft-link">
                        <li style={{backgroundColor: activeNormal === 1 && "#3b3b3b", borderLeft: activeNormal === 1 && "3px solid #f36196"}} class={active === 1 ? "navbaraccount-li has-subnav nav-active" : "navbaraccount-li has-subnav"}>
                            <i style={{fontSize: "14px", marginTop: "13px", color: "#ededed"}} class="fa fa-signal fa-2x"/>
                            <span style={{color: activeNormal === 1 && "#f36196"}} class="nav-text">
                    ...
                    {user.role === 1 && <Link to={"/admin/general"} onClick={() => activeNormal = 6} className="navbarleft-link">
                        <li style={{backgroundColor: activeNormal === 6 && "#3b3b3b", borderLeft: activeNormal === 6 && "3px solid #f36196"}} class={active === 7 ? "navbaraccount-li has-subnav nav-active" : "navbaraccount-li has-subnav"}>
                            <i style={{fontSize: "14px", marginTop: "13px", color: "#ededed"}} class="fa fa-user-cog fa-2x"/>
                            <span style={{color: activeNormal === 6 && "#f36196"}} class="nav-text">
                                Admin
                            </span>
                        </li>
                    </Link>}
            ...
            return (
                <div>
                    ...
                    {isLogged && usersOnline && usersOnline.length > 0 && <div className="navbarleft-title">
                        {tyExpanded ? <h5 className={tyExpanded === false && "notvisible"}>CANALES QUE SIGO</h5> : <i style={{color: "#ededed", position: "relative", left: "-10px"}} class="fas fa-video"></i>}
                    </div>}

                    
                    {usersOnline && usersOnline.map((streamer) => user.following && user.following.map((follower) => streamer.streamerId === follower && <UserOnline tyExpanded={tyExpanded} viewers={streamer.viewers} thumb={streamer.stream_thumbnail} image={streamer.streamer_avatar} streamer={streamer.streamer} title={streamer.stream_title} category={streamer.stream_category} thum={streamer.stream_thumbnail} />))}
                    {usersOnline && usersOnline.map((you) => you.streamerId === user._id && <UserOnline tyExpanded={tyExpanded} viewers={you.viewers} image={user.avatar} streamer={you.streamer} title={you.stream_title} category={you.stream_category} thum={you.stream_thumbnail}/>)}

                    <div className="navbarleft-title">
                        {tyExpanded ? <h5 className={tyExpanded === false && "notvisible"}>CANALES RECOMENDADOS</h5> : <i style={{color: "#ededed", position: "relative", left: "-10px"}} class="fas fa-video"></i>}
                    </div>

                    {recommended && recommended.map((streamer) => streamer.streamer != user.name && user.following && !user.following.includes(streamer.streamerId) && <UserOnline tyExpanded={tyExpanded} thumb={streamer.stream_thumbnail} viewers={streamer.viewers} image={streamer.streamer_avatar} streamer={streamer.streamer} title={streamer.stream_title} category={streamer.stream_category} thum={streamer.stream_thumbnail} />)}
    ...
    function getNavbarLeft() {
        if(isMobile) {
            return (
                <NavbarLeftMobile streamer={user.name}/>
            )
        } else {
            return (
            <aside  class={tyExpanded ? "main-menu-" + theme.theme + " expanded" : "main-menu-" + theme.theme}>
                    <ul className={tyExpanded ? "navbaraccount-ul" : "navbaraccount-ul-expanded"}>

                    {getNormalNavbar()}

                    </ul>
                    {<div  className={tyExpanded ? "navbarleft-footer" : "notvisible"}>
                        <div style={{width: "96%", height: "1px", backgroundColor: "#ffffff1a", marginTop: "10px", marginBottom: "10px"}}/>

                        <div className="navbarleft-terms">
                            <p style={{fontSize: "12px"}}><a style={{cursor: "pointer"}}>Política de privacidad</a> <a class="vl"/> <a style={{cursor: "pointer"}}>Términos de servicio</a> <a class="vl"/> <a style={{cursor: "pointer"}}>Términos de Servicio de Streamers</a> </p>
