...
function Navbar({ socketMain, tyExpanded, expanded, handleMessage, isMobile }) {
  ...
  setTimeout(() => { setIsLoadingMain(false) }, 400)
    ...
    if (socketMain != null) {
      socketMain.on("newMessage", (name) => {
        if (!messageData.includes(name)) {
          messageData = [...messageData, name]
          console.log(messageData)
        }
        setMessages(messageData);
        console.log("Nuevo mensaje desde navbar", messages)
      });
    }
  }, [socketMain]);
  ...
  useEffect(() => {
    if (socketMain != null) {
      socketMain.on("newNotification", (name) => {
        console.log("Nueva notification ", name)
      });
    }
  }, [socketMain]);
  ...
  const userLink = () => {
    ...
    return (
      <div className='navbar-image-avatar-container'>
        <div onClick={onMouseEnterAccount} style={{ width: dropdownAccount ? "42px" : "40px", height: dropdownAccount ? "42px" : "40px", backgroundColor: dropdownAccount && "#f36196", position: "relative", left: "  ", top: "2px" }} className='navbar-image-avatar'>
    ...
    if (dashboard === false && isLoadingMain === false) {
      return (
        <nav className={"navbar-" + theme.theme} >
          {!isMobile && <a className={"navbar-logo-" + theme.theme} onClick={closeMobileMenu}>
            <i onClick={() => clickPulsedButton()} style={{ fontSize: "18px", zIndex: "1000", marginLeft: "5px", transform: expanded === false && "rotate(90deg)", transition: "0.2s", color: "#ededed" }} class="fas fa-bars" />
          </a>}
          <Link to="/" className={"navbar-logo-" + theme.theme} onClick={closeMobileMenu}>
            <img src='/images/logo.png' style={{ width: "145px" }} alt="" />
          </Link>



          {!isMobile && <div style={{ width: isMobile ? "50%" : "100%", alignItems: "center", height: "50px", marginRight: "5px" }}>
          ...
          <ul className={'nav-menu'}>
            ...
            <div style={{ display: "flex", alignItems: "center", position: "relative", left: isLogged ? "20px" : "0px" }}>
              ...
              {isLogged && <div style={{ display: "flex", alignItems: "center", gap: '15px' }}>
                ...
                {searchMobile === true && <Tippy placement="bottom" theme="pinkker" content={<h1 style={{ fontSize: "12px", fontFamily: "Montserrat" }}>Mensajes</h1>}>
                  <li onClick={onMouseEnterMessage} className={'nav-item-' + theme.theme + " pinkker-button-more"}>
                    <a className={"nav-links-" + theme.theme} onClick={closeMobileMenu}>
                      {messages && messages.length != 0 && <div className='navbar-numbers'>{messages.length}</div>}
                      <img style={{ width: isMobile ? "30px" : "17px" }} src="/images/iconos/mensaje.png" />
                    </a>
                  </li>
                </Tippy>}
                {
              ...
              {!isLogged &&
                <h6 onClick={() => togglePopupAuth(0)} className="button-navbar-login">Iniciar Sesión</h6>
              }

              {!isLogged &&
                <h6 onClick={() => togglePopupAuth(1)} className="button-navbar-register">Registrarse</h6>
              }
            ...
            {dropdownBalance && <DropdownBalance closeNavbar={closeMobileMenu} />}

            {dropdownNews && <DropdownNews closeNavbar={closeMobileMenu} />}
            {dropdownNotifications && <DropdownNotifications closeNavbar={closeMobileMenu} />}
            {dropdownFriends && <DropdownFriends isMobile={isMobile} closeNavbar={closeMobileMenu} />}

            {dropdownMessage && <DropdownMessage handleMessage={(e) => handleMessage(e)} closeNavbar={closeMobileMenu} />}
            {dropdownPurchase && <DropdownPurchase closeNavbar={closeMobileMenu} />}

            {/* {isLogged && userLink()} */}
            {dropdownAccount && <DropdownAccount toggleLang={() => {
              setDropdownLang(true)
              setDropdownAccount(false)
            }} closeNavbar={() => {
              setDropdownLang(false)
            ...
            {dropdownLang && <DropdownLang closeNavbar={closeMobileMenu} close={() => setDropdownLang(false)} callback={() => setDropdownAccount(false)} />}

          </ul>

          {showPopupAuth === true && <Auth isMobile={isMobile} typeDefault={type} closePopup={() => togglePopupAuth()} />}
        </nav>
      )
    } else {
      return (
        <nav className={"navbar-" + theme.theme}>
          <a className={"navbar-logo-" + theme.theme} onClick={closeMobileMenu}>
            <i onClick={tyExpanded} style={{ fontSize: "16px" }} class="fas fa-bars">
            </i>
          </a>
        </nav>
      )
