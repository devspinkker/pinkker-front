...
function DropdownMessage( { closeNavbar, handleMessage } ) {
  ...
  const renderMessages = (message) => {
        ...
        <li onClick={() => openHandleMessage(message.to)}>
          <div className='dropdownmessage-news-card'>
            ...
            <div style={{marginLeft: "5px", width: "90%"}}>
                  <h3>{message.to}</h3>
                  {message.viewed === false && <p> <a style={{color: "darkgray"}}>Enviado hace {dateStringSince(message.createdAt)}</a></p>}
                  {message.viewed === true && <p> <a style={{color: "darkgray"}}>Visto hace {dateStringSince(message.viewedDate)}</a></p>}
    ...
    return (
      <li style={{paddingLeft: "5px"}} onClick={() => openHandleMessage(message.name)}>
        <div className='dropdownmessage-news-card'>
          <div>
                <div style={{width: "45px", position: "relative"}} className='navbar-image-avatar-container'>
                  <div style={{backgroundColor: "transparent", width: "45px"}} className='navbar-image-avatar'>
                    <a href={'/' + message.name}><img style={{backgroundColor: "#080809"}} src={message.avatar} alt="" /></a>
                  </div>
                </div>
          </div>
          <div style={{marginLeft: "5px", width: "90%"}}>
                <h3>{message.name}</h3>
                <p> <a style={{fontWeight: "600", letterSpacing: "0.7px"}}>{message.message}</a> • <a style={{color: "darkgray"}}>{dateStringSince(message.createdAt)}</a></p>
          </div>
          {message.viewed === false && <div style={{display: "flex", alignItems: "center", fontSize: "34px", color: "#f36196" , marginRight: "20px"}}>
            •
          </div>}
        </div>
    </li>           
  ...
  return (
    <>
      <ul ref={divRef} className={click ? 'dropdownmessage-menu clicked' : 'dropdownmessage-menu'}>
        ...
        <div style={{width: "99%"}} className="dropdowncomunidad-container">
            ...
            <div style={{ height: "175px"}}>
              {messages != null && messages.length > 0 &&  messages.map((message) => renderMessages(message))}
              {messages === null && <div style={{ minHeight: "150px", display: "flex", alignItems: "center", justifyContent: "center"}}>
                <ScaleLoader width={4} height={20} color="#f36197d7" />
                  </div>}

              {messages != null && messages.length === 0 && <div style={{ minHeight: "150px", display: "flex", alignItems: "center", justifyContent: "center"}}>
            ...
            <Link style={{textDecoration: "none"}} to="/direct/inbox">
                <li style={{borderTop: "1px solid #4b4b4b8f"}}>
                    <div style={{color: "#f36196"}} className="dropdownchatconfig-link" onClick={closeNavbar}> 
                        Ver más
                        <i style={{marginLeft: "10px", marginTop: "3px"}} class="fas fa-angle-down"></i>
                    </div>
                </li>
            </Link>
