...
import { fetchSearch, dispatchGetSearch } from "../../../redux/actions/searchAction";

import SearchDropdown from './dropdown/SearchDropdown';

import { addHistorySearch, existsHistoryChannelName, getHistorySearch, removeHistorySearch } from '../../../helpers/searchHelper';
...
function useOnClickOutside(ref, handler) {
    useEffect(() => {
      const listener = event => {
        ...
        if (event.target.parentElement?.className === 'dropdownsearch-close-icon') {
            return;
          }
        handler(event);
      };
...
export default function Search({isMobile}) {
    ...
    const handleKey = (e) => {
        if (e.key === 'Enter') {
            if(text === null) {
                alert({type: "ERROR", message: "Rellena algo en el campo de busqueda!"});
                return;
            }
    ...
    return (
        <div ref={divRef} className={search === null ? 'navbar-search-'+ theme.theme + ' navbar-search-margin' : 'navbar-search-'+ theme.theme}>
            ...
            {search != null && <SearchDropdown isMobile={isMobile} handleRemove={(e) => removeHistory(e)} handleClickHistory={(name, avatar) => handleClick(name, avatar)} search={search} text={text}/>}
