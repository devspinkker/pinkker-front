            ...
            if(followParam === false) {
                return (
                    <Tippy theme="pinkker" content={<h1 style={{fontSize: "12px", fontFamily: "Montserrat"}}>Empezar a seguir</h1>}>
                        <button onClick={() => followUser()} style={{ marginTop: "0px", fontSize: "12px"}} className="channel-bottom-v2-button-follow">Seguir</button>    
                    </Tippy>
                )
            } else {
                return (
                    <Tippy theme="pinkker" content={<h1 style={{fontSize: "12px", fontFamily: "Montserrat"}}>Dejar de seguir</h1>}>
                        <button 
                            style={{marginTop: "0px", width: "100px", marginLeft: "5px", marginRight: "5px", backgroundColor: nameD === "Siguiendo" && "#762543"}} 
                            onMouseEnter={() => setNameD("Dejar de seguir")} 
                            onMouseLeave={() => setNameD("Siguiendo")} 
                            onClick={() => unfollowUser()} 
                            className="followerscard-button-unfollow">{nameD}</button>
                    </Tippy>
                )
            }    
        }
    ...
    return (
        <div className="muro-tweet-secondary-follow-card">
            <div style={{display: "flex"}}>
                <img onClick={() => onClickChangeRoute("/" + followData.name)} style={{width: "35px", borderRadius: "50px", marginRight: "10px", cursor: "pointer"}} src={followData.avatar} />
                <div>
                    <h3 onClick={() => onClickChangeRoute("/" + followData.name)}>{followData.name}</h3>
                    <p style={{color: "darkgray", fontSize: "13px"}}>{followData.followers.length} seguidores</p>
                </div>
            </div>
            {getFollowButton()}
        </div>
