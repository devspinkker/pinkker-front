...
export default function ViewTweet({ closePopup, tweet, isLiked, isRetweet }) {
    ...
    function createComment() {
        if(comment.trim() != '') {
            addComment(token, comment, tweet._id, 1)
            .then(res => {
                ...
                <div>
                    <div style={{display: "flex"}}>
                        <div style={{textAlign: "left"}} className="tweetcard-avatar">
                            <img style={{width: "45px", borderRadius: "100px"}} src={tweet.avatar}/>
                        </div>
    
                        <div style={{position: "relative", left: "-10px", textAlign: "left"}} className="tweetcard-primary">
                            <h3>{tweet.name}</h3>
                            <p style={{color: "lightgray", fontSize: "15px"}}>@{tweet.user}</p>
                        </div>
                    </div>
    
    
                        <div style={{marginTop: "10px", display: "flex", justifyContent: "left", marginBottom: "10px"}}>
                            <p style={{fontSize: "24px", color: "white"}}>{tweet.text}</p>
                        </div>
                        <div style={{marginTop: "10px"}}>
                            <img style={{borderRadius: "20px"}} src={tweet.photo} alt="" />
                        </div>
                        <div style={{marginTop: "10px", display: "flex", justifyContent: "left", marginBottom: "10px"}}>
                            <p style={{color: "#f36196"}}>Contenido de Suscriptores</p>
                        </div>
                        <div style={{marginTop: "10px", display: "flex", justifyContent: "left", marginBottom: "10px"}}>
                            <p style={{fontSize: "14px", color: "darkgray"}}>1:59 p. m. · 10 nov. 2022</p>
                        </div>
    
                        <div style={{width: "100%", margin: "10px auto", height: "1px", backgroundColor: "#ffffff1a", marginTop: "15px", marginBottom: "15px"}}/>
    
    
                        <div className="viewtweet-quantity">
                            <div className="viewtweet-quantity-card">
                                <p style={{color: "white"}}>{tweet.likes.length} <a style={{color: "darkgray"}}>Me gusta</a></p>
                            </div>
                        </div>
    
                        <div style={{width: "100%", margin: "10px auto", height: "1px", backgroundColor: "#ffffff1a", marginTop: "15px", marginBottom: "15px"}}/>
    
                        <div style={{width: "70%", margin: "0 auto", textAlign: "center"}} className="tweetcard-icons">
                            ...
                            <div onClick={() => handleLike()} style={{color: isLiked && "red"}} className="tweetcard-icon-like">
                                {isLiked ? <i style={{fontSize: "16px"}} class="fas fa-heart"/> : <i style={{fontSize: "16px"}} class="far fa-heart"/>}
                            </div>
            ...
            <div>
                <div style={{display: "flex"}}>
                    ...
                    {tweet.image != null && <div style={{marginTop: "10px"}}>
                        <img style={{borderRadius: "20px", minWidth: "250px", maxWidth: "250px"}} src={tweet.image} alt="" />
                    </div>}
                    <div style={{marginTop: "10px", display: "flex", justifyContent: "left", marginBottom: "10px"}}>
                        <p style={{fontSize: "14px", color: "darkgray"}}>1:59 p. m. · 10 nov. 2022</p>
                    </div>
                    ...
                    <div className="viewtweet-quantity">
                        <div className="viewtweet-quantity-card">
                            <p style={{color: "white"}}>{tweet.totalRetweets} <a style={{color: "darkgray"}}>Retweets</a></p>
                        </div>
                        <div className="viewtweet-quantity-card">
                            <p style={{color: "white"}}>{tweet.totalLikes} <a style={{color: "darkgray"}}>Me gusta</a></p>
                        </div>
                    </div>
                    ...
                    <div style={{width: "70%", margin: "0 auto", textAlign: "center"}} className="tweetcard-icons">
                        ...
                        <div onClick={() => handleRetweet()} style={{color: isRetweet && "rgb(46, 255, 60)"}} className="tweetcard-icon-retweet">
    ...
    function renderComments() {
        return (
            <div className="viewtweet-comments-container">
                {comments != null ? comments.map((comment) => <TweetCard tweet={comment}/>) : <div style={{ minHeight: "150px", display: "flex", alignItems: "center", justifyContent: "center"}}>
                ...
                <div className={'viewtweet-popup-container'}>
                    ...
                    <div style={{display: "flex", alignItems: "center", marginTop: "20px", marginBottom: "10px"}}>
                        ...
                        <div style={{marginLeft: "10px", height: "50px"}} className="muro-send-tweet-input">
                            <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Twittea tu respuesta" type="text" />
                        </div>
