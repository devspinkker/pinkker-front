...
export default function AddPopupGallery({ closePopup, reloadData }) {
  ...
  async function handleSubmit() {
    ...
    const res = await axios.post(
      `${process.env.REACT_APP_DEV_API_URL}/gallery/upload_gallery?title=${title}&description=${description}`,
      file,
      {
        headers: {
  ...
  return (
    <div style={{ zIndex: "9999" }} className="usersettings-popup-body">
      <div
        style={{ textAlign: "left", width: "25%", borderRadius: "5px" }}
        ...
        {loading === false ? (
          <div>
            ...
            <div style={{ cursor: "pointer" }} className="popupgallery-card">
              <div>
                <i style={{ fontSize: "24px" }} className="fas fa-upload"></i>
                <h4 style={{ fontFamily: "Poppins", letterSpacing: "1px" }}>
                  CARGAR FOTO
                </h4>
                <input
                  className="popupgallery-loadfile"
                  type="file"
          ...
          >
            <div style={{ textAlign: "center" }}>
              <h3 style={{ marginBottom: "10px" }}>
                Subiendo la imagen... espera porfavor
              </h3>
