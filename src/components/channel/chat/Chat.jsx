...
const Chat = ({external, socket, socketMain, chatExpanded, announce, handleAnnounce, callback, handleSendMessage, setUserSuscripted, setSuscribers, isMobile}) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const dataDonationsSubscriptions = await getStreamerDonationSubscription(streamer, page);
      if(dataDonationsSubscriptions != null && dataDonationsSubscriptions != undefined) {
  ...
  const loadDataOnlyOnce = () => {
    ...
    if(socket != null && socket != undefined) {
      if(user != null && user != undefined && user != []) {
        socket.emit("join", { _id: user._id, name, room, color: user.color, lookImage: user.lookImage, birthDate: user.birthDate, pinkkerPrime: user.pinkkerPrime?.active }, () => {
          setMessages([{user: "System", text: "Te damos la bienvenida a la sala de chat."}]);
          messageData = [{user: "System", text: "Te damos la bienvenida a la sala de chat."}]
  ...
  useEffect(() => {
    socket.on("onClearChat", (user) => {
      setMessages([{user: "System", text: user + " ha despejado el chat de esta sala"}]);
      messageData = [{user: "System", text: user + " ha despejado el chat de esta sala"}];
    });
    ...
    if(message.startsWith("/")) {
      ...
      if(commandName === "mod") {
        if(commandArgs.length === 2) {
          ...
          setMessage("")
          socket.emit("sendMessageSystem", "Ahora " + commandArgs[1] + " es mod de la sala");
      ...
      if(commandName === "unmod") {
        if(commandArgs.length === 2) {
          ...
          setMessage("")
          socket.emit("sendMessageSystem", "Ahora " + commandArgs[1] + " no es mod de la sala");
          socket.emit("sendReload", () => setMessage(""));
          return;
        }
      }
      ...
      if(commandName === "vip") {
        if(commandArgs.length === 2) {
          ...
          setMessage("")
          socket.emit("sendReload", () => setMessage(""));
          socket.emit("sendMessageSystem", "Ahora " + commandArgs[1] + " es vip de la sala");
        }
        return;
      }
      ...
      if(commandName === "unvip") {
        if(commandArgs.length === 2) {
          ...
          setMessage("")
          socket.emit("sendReload", () => setMessage(""));
          socket.emit("sendMessageSystem", "Ahora " + commandArgs[1] + " no es vip de la sala");
        }
        return;
      }
      ...
      if(commandName === "slow") {
        if(commandArgs.length === 2) {
          ...
          setMessage("")
          socket.emit("sendReload", () => setMessage(""));
          socket.emit("sendMessageSystem", "Ahora hay " + commandArgs[1] + " segundos de delay en la sala");
        }
        return;
      }
      ...
      if(commandName === "slowoff") {
        ...
        setMessage("")
        socket.emit("sendReload", () => setMessage(""));
        socket.emit("sendMessageSystem", "Ahora hay 2 segundos de delay en la sala"); 
        return;
      }
      ...
      if(commandName === "ban") {
        if(commandArgs.length === 3) {
          ...
          setMessage("")
          socket.emit("sendReload", () => setMessage(""));
          socket.emit("sendMessageSystem", "Ahora " + commandArgs[1] + " esta baneado de la sala");
        }
        return;
      }
      ...
      if(commandName === "unban") {
        if(commandArgs.length === 2) {
          ...
          setMessage("")
          socket.emit("sendReload", () => setMessage(""));
          socket.emit("sendMessageSystem", "Ahora " + commandArgs[1] + " no esta baneado de la sala");
        }
        return;
      }
      ...
      if(commandName === "host") {
        if(commandArgs.length === 2) {
          socket.emit("sendMessageSystem", "Ahora " + commandArgs[1] + " es el host de la sala");
      ...
      if(commandName === "uniquechat") {
        ...
        socket.emit("sendMessageSystem", "Ahora esta activado el modo de chat único en esta sala.");
      ...
      if(commandName === "uniquechatoff") {
        ...
        socket.emit("sendMessageSystem", "Ahora esta desactivado el modo de chat único en esta sala.");
      ...
      if(commandName === "emoteonly") {
        ...
        socket.emit("sendMessageSystem", "Ahora esta activado el modo Solo emoticonos en esta sala.");
      ...
      if(commandName === "emoteonlyoff") {
        ...
        socket.emit("sendMessageSystem", "Ahora esta desactivado el modo Solo emoticonos en esta sala.");
      ...
      if(commandName === "subscribers") {
        ...
        socket.emit("sendMessageSystem", "Ahora esta activado el modo Solo suscriptores en esta sala.");
      ...
      if(commandName === "subscribersoff") {
        ...
        socket.emit("sendMessageSystem", "Ahora esta desactivado el modo solo suscriptores en esta sala.");
      ...
      if(commandName === "followers") {
        if(commandArgs.length === 2) {
          ...
          socket.emit("sendMessageSystem", "Ahora esta activado el modo Solo seguidores durante " + time + " minuto en esta sala.");
      ...
      if(commandName === "followersoff") {
        ...
        socket.emit("sendMessageSystem", "Ahora esta desactivado el modo solo seguidores en esta sala.");
  ...
  const handleHotel = () => {
    window.open("http://localhost:3002?name=" + user.name, 'newwin', "width=1200,height=800");
  }
      ...
      return (
          <div className="messages-wrapper">
            ...
            <Input
              ...
              callback={(amount, text, donation) => sendDonation(amount, text, donation)}
            />
          </div>
      )
    }
    ...
    if(type === 1) {
      return (
          <div style={{overflow: "scroll"}} className="messages-wrapper">
            <AccordionList users={users} mods={mods} vips={vips} streamer={streamer}/>
          </div>
      )
    }
  }
  ...
  function getDonationSubscriptionCard(donation) {
    return (
      <div style={{display: "flex", alignItems: "center", backgroundColor: "#534bb3", padding: "5px", borderRadius: "10px"}}>
        <img style={{width: "15px"}} src="https://static.twitchcdn.net/assets/GiftBadge-Gold_72-6e5e65687a6ca6959e08.png"/>
        <div style={{marginLeft: "5px", display: "flex", alignItems: "center"}}>
          <p style={{lineHeight: "10px", fontSize: "14px", fontWeight: "800"}}>{donation.userName}</p>
          <p style={{fontSize: "14px", fontWeight: "800", color: "violet", marginLeft: "5px"}}>{donation.amount}</p>
        </div>
      </div>
    )
  }
  ...
  function getDonation() {
      ...
      return (
        <div className="chat-donation-body">
          <div className="chat-donation-container">
            <div className="chat-donation-card-container" style={{width: "90%", margin: "0 auto", display: "flex", alignItems: "center", height: "60px"}}>
              {donationsSubscriptions.length > 0 || donations.length > 0 ?
                <div style={{display: "flex", alignItems: "center"}}>
                  {donationsSubscriptions.map((donation, index) => donation.enabled === true && getDonationSubscriptionCard(donation))}
                  {donations.map((donation, index) => donation.enabled === true && <Donation callback={() => console.log("Expiroo ")} donation={donation} onClick={() => toggleDonationCard(donation.amount, donation.userName, donation.userAvatar, donation.text, donation.userLook, donation.userColor)} />)}
                </div>
                : <div style={{textAlign: "center", width: "100%"}}><h5 style={{color: "#f36196", fontFamily: "'Roboto', 'Arial', sans-serif", fontSize: "13px", fontWeight: "600", letterSpacing: "0.5px"}}>¡Suscríbete, regala suscripciones o envía Pixels para aparecer aquí!</h5></div>}
    ...
    return (
      <div style={{height: "60px", backgroundColor: "#0404048f", display: "flex", alignItems: "center", justifyContent: "center"}}>
  ...
  function getChat() {
    return (
      <div className="chat-wrapper">
        ...
        {betTopOpen && bets && <BetTop bets={bets} betWon={betWon} handleBet={() => toggleBetDropdown()} />}
        {!betTopOpen && betWon && <BetTopWin betWonAmount={betWonAmount} betWon={betWon} />}

        <div className="list-wrapper">
          <div style={{display: "flex", alignItems: "center", justifyContent: "center", padding: "10px", fontFamily: "Montserrat"}}>
            
            <Tippy placement="right-start" theme="pinkker" content={<h1 style={{fontSize: "12px", fontFamily: "Montserrat"}}>Contraer</h1>}>
              <img  onClick={() => chatExpanded()} style={{width: "12px", cursor: "pointer"}} className="chat-button-more" src="/images/iconos/contraer.png"  />

            </Tippy>
            
            <h4 style={{fontSize: "12px", width: "290px", textAlign: "center", letterSpacing: "0.5px"}}>CHAT DEL DIRECTO</h4>
            ...
            {type === 1 && <Tippy placement="left-start" theme="pinkker" content={<h1 style={{fontSize: "12px", fontFamily: "Montserrat"}}>Chat</h1>}>
              <i onClick={() => setType(0)} style={{cursor: "pointer", width: "0px", textAlign: "center"}} class="fas fa-arrow-left pinkker-button-more"/>
            </Tippy>}
        ...
        <div style={{width: "100%", height: "1px", backgroundColor: "#2b2b2b3f", marginTop: "19px auto"}}/>

        {getDonation()}
        {donationCard && <DonationCard donationColor={donationColor} donationAmount={donationAmount} donationName={donationName} donationAvatar={donationAvatar} donationText={donationText} donationLook={donationLook} close={() => setDonationCard(false)} /> }
  ...
  function finishBetSetWinner(typeWin) {
    ...
    const userBet = bets.participants.find(bet => bet.userId.toString() === user._id.toString());
    if(userBet) {
  ...
  return (
    <div className="page-wrapper">
      {getChat()}
      {betWon && <div style={{position: "fixed", width: "0%", height: "0%", top: "0", right: "320px"}}>
                <Confetti
                    numberOfPieces={100}
                    width={"300px"}
                    height={"1080px"}
      ...
      {predictPopup && <PredictPopup finishBetGeneral={(typeWin) => finishBetSetWinner(typeWin)} reloadBets={() => sendBetReload()} bets={bets} streamer={streamer} closePopup={() => setPredictPopup(false)} />}
    </div>
  );
};
