...
export default function Vod(props) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const data = await getStreamerVod(props.streamer, props.limit, props.sort);
    ...
    return (
        <div className='channel-vod-body'>
            {videos != null && videos != undefined && videos.length > 0 && videos.map((video) => isLoading ? <CardSkeleto/> :
                <Link style={{textDecoration: "none"}} to={"/vod/" + video._id}><VodCard width="260px" image={video.stream_thumbnail} title={video.stream_title} categorie={video.stream_category} tags={video.stream_tag}/></Link>
            )}
        </div>
    )
}