...
export const getStreamsOnline = async (limit) => {
  try {
    let response = await axios.get(
      `${url}/stream/get_streams_online?limit=${limit}`
    );
    return response.data;
  } catch (error) {
    console.log("Error while calling getStreamerStream", error);
...
export const getStreamsByCategorie = async (categorie) => {
  try {
    let response = await axios.get(
      `${url}/get_streams_by_categorie?categorie=${categorie}`
    );
    return response.data;
  } catch (error) {
    console.log("Error while calling getStreamerStream", error);
...
export const getViewersStream = async (streamer) => {
  try {
    let response = await axios.get(
      `${process.env.REACT_APP_DEV_CHAT_URL}/api/user/get_users_in_room?room=${streamer}`
    );
    return response.data;
  } catch (error) {
