...
export const getBetsStreamer = async (token, streamer) => {
    try {
        ...
        console.log('Error while calling getBetsStreamer', error);
    }
}

export const createBet = async (token, streamer, title, resultOne, resultTwo, minParticipate) => {
    try {
        const res = await axios.post(`${url}/createBet`, {
            streamer, title, resultOne, resultTwo, minParticipate
        }, {
            headers: {Authorization: token}
        })
        return res
...
export const participateBet = async (token, streamer, amount, type) => {
    try {
        const res = await axios.post(`${url}/participateBet`, {
            streamer, amount, type
        }, {
            headers: {Authorization: token}
        })
        return res
...
export const finishBet = async (token, streamer, typeWin) => {
    try {
        const res = await axios.post(`${url}/finishBet`, {
            streamer, typeWin
        }, {
            headers: {Authorization: token}
        })
...
export const restoreBet = async (token) => {
    try {
        const res = await axios.post(`${url}/restoreBet`, {
